...
MongoDOWN.prototype._open = function (options, callback) {
  ...
  var connect = function () {
    var stripped_location = self.location.split('/');
    self.coll = stripped_location.pop();
    stripped_location = stripped_location.join('/');
    self._db = mongojs(stripped_location, [self.coll]);
    callback(null, self);
  };
...
MongoDOWN.prototype._get = function (key, options, callback) {
  this._db[this.coll].findOne({ _id: key }, function (err, doc) {
    if (err) return callback(err);
...
MongoDOWN.prototype._put = function (key, value, options, callback) {
  this._db[this.coll].update({ _id: key }, { _id: key, value: value }, { upsert: true }, callback);
};

MongoDOWN.prototype._del = function (key, options, callback) {
  this._db[this.coll].remove({ _id: key }, callback);
};
    ...
    switch (batch[0].type) {
      case 'put':
        ...
        for (var n = 0, l = batch.length; n < l; n++)
          self._db[self.coll].save({ _id: batch[n].key, value: batch[n].value }, next());
        break;
      case 'del':
        var keys = batch.map(function (e) { return e.key; });
        self._db[self.coll].remove({ _id: { $in: keys } }, commit);
        break;
...
MongoDOWN.prototype._approximateSize = function (start, end, callback) {
  this._db[this.coll].count({ _id: { $gte: start, $lte: end } }, callback);
};
...
var MongoIterator = function (db, options) {
  ...
  this._cursor = db._db[db.coll].find(query).sort({ _id: options.reverse ? -1 : 1 });
